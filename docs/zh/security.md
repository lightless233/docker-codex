...
修改 checkout、Git metadata 或共享 Codex home（包括 `/codex-home` 兼容
别名）的所有权，因此普通命令在 bind mount 中创建的文件仍属于宿主用户。
...
只有兼容链接。选择 `--profile` 时，启动器只把当前 profile 的独立目录读写
挂载到链接目标，以允许 Codex 原子持久化信任与其他交互配置；其他托管
profile 在容器内保持不可达。当前 Codex 和其命令能读写所选 profile 中的
